...
export default {
  ...
  methods: {
    onCreate() {
      const routeUrl = this.$router.resolve({ name: "StatefulSetCreate", params: { operation: "create" }, query: { yamlShow: false, cluster: this.clusterName } })
      window.open(routeUrl.href, "_blank")
    },
    openDetail(row) {
      const routeUrl = this.$router.resolve({ 
    ...
    yamlCreate() {
      const routeUrl = this.$router.resolve({ name: "StatefulSetCreateYaml", params: { operation: "create" }, query: { type: "statefulsets", cluster: this.clusterName } })
      window.open(routeUrl.href, "_blank")
    },
    onDelete(row) {
